...
class SCST(nn.Module):
    ...
    def forward_greedy(self, questions_ids, questions_mask, answers_ids, answers_mask, images, model, images_mask=None):
        ...
        if (greedy_input_ids == 0).any():  # Check if there are any zeros in the tensor
            row, col = (greedy_input_ids == 0).nonzero(as_tuple=True)  # Get row and column indices of zeros
            hyp = greedy_input_ids[:, col[0]:]  # Slice from the first zero in the first row (preserving 2D)
        else:
    ...
    def forward_sampling(self, questions_ids, questions_mask, answers_ids, answers_mask, reward_greedy, images, model, images_mask=None):
