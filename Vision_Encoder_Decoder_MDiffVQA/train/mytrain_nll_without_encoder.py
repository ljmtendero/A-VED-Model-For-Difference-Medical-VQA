...
from mymodels.bertdecoder import BertDecoder

# from mydatasets.mimic_dataset import mimic_Dataset
from mydatasets.mimic_dataset_for_labels import mimic_Dataset
...
for epoch in range(epochs):
    ...
    with tqdm(iter(train_dataloader), desc="Train", unit="batch") as tepoch:
        for steps, batch in enumerate(tepoch):
            ...
            input_ids = batch['input_ids_with_answers'].to(device)
            attention_mask = batch['input_ids_with_answers_attention_masks'].to(device)
    ...
    with torch.no_grad():
        ...
        with tqdm(iter(test_dataloader), desc="Metrics for Test", unit="batch") as tepoch:
            for steps, batch in enumerate(tepoch):
                input_ids = batch['input_ids'].to(device)
                input_ids_with_answers = batch['input_ids_with_answers'].to(device)
                attention_mask = batch['input_ids_with_answers_attention_masks'].to(device)
                labels = batch['labels'].to(device)

                # Text Decoder
                nll_loss = model.decode(
                    input_ids=input_ids_with_answers,
                    attention_mask=attention_mask,
                    labels=labels,)                
                loss = nll_loss
